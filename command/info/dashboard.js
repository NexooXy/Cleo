...
module.exports = {
	...
	async run({ msg, conn }) {
		...
		for(let i = 0; i < dbny; i++){
		  txt += `*乂 Cmd : ${dashboard[i].name}*\n`
		  txt += monospace(`   × Total : ${dashboard[i].success + dashboard[i].failed}`) + `\n`;
		  txt += monospace(`   × Sukses : ${dashboard[i].success}`) + `\n`;
		  txt += monospace(`   × Gagal : ${dashboard[i].failed}`) +`\n`;
		  txt += monospace(`   × Terakhir digunakan : ${await prettyms(Date.now() - dashboard[i].lastUpdate, {
				verbose: true,
			})}`) + `\n\n`;
		}
