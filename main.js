...
async function start(){
  ...
  console.log(color('------------------------------------------------------------------------', 'white'))
  console.log(color('[PEMILIK BOT]', 'aqua'), color(config.author, 'magenta'))
  console.log(color('[BOT]', 'aqua'), color('Cleo Bot Online!', 'magenta'))
  ...
  const decodeJid = (jid) => {
	...
	conn.ev.on("connection.update", async (ktl) => {
	  ...
	  if (connection == "open") {
	    console.log(chalk.yellow("Sukses Terhubung ke whatsapp"))
	    conn.sendMessage(config.owner[0],{text: "_*Bot Sekarang Online!*_"})
	  }
	  if (connection === "close") {
